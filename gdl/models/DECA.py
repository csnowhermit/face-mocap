...
class DecaModule(LightningModule):
    ...
    def forward(self, img):    # args1 batch --> img
        # values = self.encode(img, training=False)
                ...
                if self.deca.config.detail_constrain_type == 'exchange':
                    ...
                    expcode, posecode, shapecode, lightcode, texcode, images, cam, lmk, masks, va, expr7, affectnetexp, detailcode, detailemocode, exprw = \
                        self._expression_ring_exchange(original_batch_size, K,
                                                       ...
                                                       images, cam, lmk, masks, va, expr7, affectnetexp, detailcode,
                                                       detailemocode, exprw)
        ...
        effective_batch_size = images.shape[
            0]  # this is the current batch size after all training augmentations modifications

        # 1) Reconstruct the face mesh
        # FLAME - world space
        # 直接解析矩阵的话，shape置为0，pose用原来的
        shapecode = torch.zeros([1, 100], dtype=torch.float32)
        # posecode = torch.zeros([1, 6], dtype=torch.float32)
        shapecode = shapecode.cuda()
        # posecode = posecode.cuda()
        ...
        if self.deca._has_neural_rendering():
            ...
            if self.mode == DecaMode.DETAIL:
                ...
                translated_uv = F.grid_sample(torch.cat([predicted_detailed_translated_image, masks], dim=1),
    ...
    def encode(self, img, training=True) -> dict:
        """
        ...
        # 2) DETAIL STAGE
        if self.mode == DecaMode.DETAIL:
            all_detailcode = self.deca.E_detail(images)    # [1, 128]

            # identity-based detail code
            detailcode = all_detailcode[:, :self.deca.n_detail]    # [1, 128]

            # detail emotion code is deprecated and will be empty
            detailemocode = all_detailcode[:, self.deca.n_detail:(self.deca.n_detail + self.deca.n_detail_emo)]    # [1, 0]
