...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        # set up parameters
        self.n_param = model_cfg.n_shape + model_cfg.n_tex + model_cfg.n_exp + model_cfg.n_pose + model_cfg.n_cam + model_cfg.n_light    # 100+50+50+6+3+27=236
        ...
        # encoders
        self.E_flame = ResnetEncoder(outsize=self.n_param).to(self.device)    # 这里self.n_param已经累加了新加的expression
    ...
    def displacement2normal(self, uv_z, coarse_verts, coarse_normals):
        ...
        uv_coarse_vertices = self.render.world2uv(coarse_verts).detach()    # [1, 3, 256, 256]
        uv_coarse_normals = self.render.world2uv(coarse_normals).detach()    # [1, 3, 256, 256]
    
        uv_z = uv_z*self.uv_face_eye_mask
        uv_detail_vertices = uv_coarse_vertices + uv_z*uv_coarse_normals + self.fixed_uv_dis[None,None,:,:]*uv_coarse_normals.detach()    # [1, 3, 256, 256]
        dense_vertices = uv_detail_vertices.permute(0,2,3,1).reshape([batch_size, -1, 3])    # [1, 65536, 3]
    ...
    def encode(self, images, use_detail=True):
        if use_detail:
            ...
            with torch.no_grad():
                parameters = self.E_flame(images)    # [1, 237]
        else:
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        ## decode
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])    # 这时候landmark2d和landmark3d均为[1, 68, 3]
        if self.cfg.model.use_tex:
            albedo = self.flametex(codedict['tex'])
        else:
            albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size], device=images.device)    # [1, 3, 256, 256]
        landmarks3d_world = landmarks3d.clone()

        ## projection
        landmarks2d = util.batch_orth_proj(landmarks2d, codedict['cam'])[:,:,:2]; landmarks2d[:,:,1:] = -landmarks2d[:,:,1:]#; landmarks2d = landmarks2d*self.image_size/2 + self.image_size/2    # projection后才是[1, 68, 2]
        ...
        ## rendering，渲染过程
        if rendering:
            ops = self.render(verts, trans_verts, albedo, codedict['light'])    # dict
            ## output
        ...
        if self.cfg.model.use_tex:    # 使用材质的话需要反射率矩阵
            opdict['albedo'] = albedo
            
        if use_detail:
            uv_z = self.D_detail(torch.cat([codedict['pose'][:,3:], codedict['exp'], codedict['detail']], dim=1))    # [1, 1, 256, 256]
            if iddict is not None:
                uv_z = self.D_detail(torch.cat([iddict['pose'][:,3:], iddict['exp'], codedict['detail']], dim=1))
            uv_detail_normals = self.displacement2normal(uv_z, verts, ops['normals'])    # [1, 3, 256, 256]
            uv_shading = self.render.add_SHlight(uv_detail_normals, codedict['light'])    # [1, 3, 256, 256]
            uv_texture = albedo*uv_shading    # [1, 3, 256, 256]，纹理图
        ...
        if vis_lmk:
            landmarks3d_vis = self.visofp(ops['transformed_normals'])#/self.image_size [1, 68, 1]
            landmarks3d = torch.cat([landmarks3d, landmarks3d_vis], dim=2)    # [1, 68, 4]
    ...
    '''
        这个decode里只有render部分，flame在
        :param codedict 必要的参数
        :param verts landmarks2d landmarks3d flame的结果直接传进来，这里landmark2d和landmark3d均为[bs, 68, 3]的
    '''
    @torch.no_grad()
    def decode_render(self, codedict, verts, landmarks2d, landmarks3d, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
               render_orig=False, original_image=None, tform=None):
        images = codedict['img']    # 原图
        batch_size = images.shape[0]


        if self.cfg.model.use_tex:    # 这里没有用纹理
            albedo = self.flametex(codedict['tex'])
        else:
            albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size],
                                 device=images.device)  # [1, 3, 256, 256]
        landmarks3d_world = landmarks3d.clone()

        # projection，这里重新做下projection
        landmarks2d = util.batch_orth_proj(landmarks2d, codedict['cam'])[:, :, :2]; landmarks2d[:, :, 1:] = -landmarks2d[:, :, 1:]  # ; landmarks2d = landmarks2d*self.image_size/2 + self.image_size/2    # projection后才是[1, 68, 2]
        landmarks3d = util.batch_orth_proj(landmarks3d, codedict['cam']); landmarks3d[:, :, 1:] = -landmarks3d[:, :, 1:]  # ; landmarks3d = landmarks3d*self.image_size/2 + self.image_size/2
        trans_verts = util.batch_orth_proj(verts, codedict['cam']); trans_verts[:, :, 1:] = -trans_verts[:, :, 1:]
        opdict = {
        ...
        if rendering:
            ...
            ## output
            opdict['grid'] = ops['grid']
            opdict['rendered_images'] = ops['images']    # [bs, 3, 224, 224]
            opdict['alpha_images'] = ops['alpha_images']
            opdict['normal_images'] = ops['normal_images']

        if self.cfg.model.use_tex:  # 使用材质的话需要反射率矩阵（这里不用材质）
            opdict['albedo'] = albedo

        if use_detail:    # 只做粗略重建
