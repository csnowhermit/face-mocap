...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        shapedirs = to_tensor(to_np(flame_model.shapedirs), dtype=self.dtype)    # [5023, 3, 400]
        shapedirs = torch.cat([shapedirs[:,:,:config.n_shape], shapedirs[:,:,300:300+config.n_exp]], 2)    # [5023, 3, 151]，这里只存需要的100个shape和51个expression
    ...
    # '''
    #     flame推理过程，通过mesh拿到landmark2d
    #     :param verts 通过矩阵计算出的mesh [bs, 5023, 3]
    #     :param pose_params deca.encode()计算出的pose参数 [bs, 6]
    #     :param eye_pose_params 眼部的pose，直接用默认的即可
    # '''
    # def forward(self, verts, pose_params=None, eye_pose_params=None):
    #     """
    #         Input:
    ...
    #     # betas = torch.cat([shape_params, expression_params], dim=1)    # [1, 100+51=151]
    #     full_pose = torch.cat([pose_params[:, :3], self.neck_pose.expand(batch_size, -1), pose_params[:, 3:], eye_pose_params], dim=1)    # [1, 15]
    ...
    #
    #     landmarks2d = vertices2landmarks(verts, self.faces_tensor,
    #                                    lmk_faces_idx,
    #                                    lmk_bary_coords)
    #     bz = verts.shape[0]
    #     landmarks3d = vertices2landmarks(verts, self.faces_tensor,
    #                                    self.full_lmk_faces_idx.repeat(bz, 1),
    #                                    self.full_lmk_bary_coords.repeat(bz, 1, 1))
    #     return verts, landmarks2d, landmarks3d
