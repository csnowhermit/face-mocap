...
def set_rasterizer(type = 'pytorch3d'):
    if type == 'pytorch3d':
        ...
        #                             '-ccbin=$$(which gcc-8)'])  # cuda10.2 is not compatible with gcc9. Specify gcc 7
        from standard_rasterize_cuda import standard_rasterize    # python setup.py install，这里直接导入，
...
class StandardRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None):
        ...
        f_vs = util.face_vertices(vertices, faces)    # [bs, 9976, 3, 3]
...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None):
        # print("self.renderer self.rasterizer:", h, w, type(h), type(w))
        fixed_vertices = vertices.clone()    # [1, 5023, 3]
        fixed_vertices[...,:2] = -fixed_vertices[...,:2]
        raster_settings = self.raster_settings
        if h is None and w is None:
            # print("1......")
            image_size = raster_settings.image_size
            rasterize_meshes_image_size = raster_settings.image_size    # 这里单独保存一份做rasterize_meshes()用
        else:
            # print("2......")
            image_size = [h, w]
            rasterize_meshes_image_size = h    # 这里单独保存一份做rasterize_meshes()用
            if h>w:
        ...
        # print("===============================================================")
        # print(type(meshes_screen),
        #       type(image_size), image_size,
        #       type(raster_settings.blur_radius),
        #       type(raster_settings.faces_per_pixel),
        #       type(raster_settings.bin_size),
        #       type(raster_settings.max_faces_per_bin),
        #       type(raster_settings.perspective_correct)
        #       )
        #
        # print("===============================================================")
        pix_to_face, zbuf, bary_coords, dists = rasterize_meshes(    # 这行报错了：CUDA error: no kernel image is available for execution on the device
            meshes_screen,
            image_size=rasterize_meshes_image_size,
            blur_radius=raster_settings.blur_radius,
            faces_per_pixel=raster_settings.faces_per_pixel,
            bin_size=raster_settings.bin_size,
            max_faces_per_bin=raster_settings.max_faces_per_bin,
            perspective_correct=raster_settings.perspective_correct,
        )    # pix_to_face[1, 224, 224, 1], zbuf[1, 224, 224, 1], bary_coords[1, 224, 224, 1, 3], dists[1, 224, 224, 1]
        vismask = (pix_to_face > -1).float()    # [1, 224, 224, 1]
        D = attributes.shape[-1]    # 12
        attributes = attributes.clone(); attributes = attributes.view(attributes.shape[0]*attributes.shape[1], 3, attributes.shape[-1])
        N, H, W, K, _ = bary_coords.shape
        mask = pix_to_face == -1    # [1, 224, 224, 1]
        pix_to_face = pix_to_face.clone()
        pix_to_face[mask] = 0
        idx = pix_to_face.view(N * H * W * K, 1, 1).expand(N * H * W * K, 3, D)
        pixel_face_vals = attributes.gather(0, idx).view(N, H, W, K, 3, D)
...
class SRenderY(nn.Module):
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        transformed_vertices[:,:,2] = transformed_vertices[:,:,2] + 10    # [16, 5023, 3]
        # attributes
        ...
        attributes = torch.cat([self.face_uvcoords.expand(batch_size, -1, -1, -1), 
                                ...
                                -1)    # [bs, 9976, 3, 12]
        # rasterize，光栅化
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)    # [bs, 13, 224, 224]
        
        ####
        # vis mask
        alpha_images = rendering[:, -1, :, :][:, None, :, :].detach()    # [bs, 1, 224, 224]

        # albedo
        uvcoords_images = rendering[:, :3, :, :]; grid = (uvcoords_images).permute(0, 2, 3, 1)[:, :, :, :2]    # [1, 3, 224, 224]
        ...
        outputs = {
            'images': images*alpha_images,    # [bs, 3, 224, 224]
            'albedo_images': albedo_images*alpha_images,    # [bs, 3, 224, 224]
            'alpha_images': alpha_images,    # [bs, 1, 224, 224]
            'pos_mask': pos_mask,    # [bs, 1, 224, 224]
            'shading_images': shading_images,    # [bs, 3, 224, 224]
            'grid': grid,    # [bs, 224, 224, 2]
            'normals': normals,    # [bs, 5023, 3]
            'normal_images': normal_images*alpha_images,    # [bs, 3, 224, 224]
            'transformed_normals': transformed_normals,    # [bs, 5023, 3]
        }
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
        ...
        if lights is None:
            ...
            )[None,:,:].expand(batch_size, -1, -1).float()    # [1, 5, 3]
            light_intensities = torch.ones_like(light_positions).float()*1.7    # [1, 5, 3]
            lights = torch.cat((light_positions, light_intensities), 2).to(vertices.device)    # [1, 5, 6]
        transformed_vertices[:,:,2] = transformed_vertices[:,:,2] + 10    # [1, 5023, 3]
        ...
        # import ipdb; ipdb.set_trace()
        # print("调用前hw：", h, w)
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes=attributes, h=h, w=w)
