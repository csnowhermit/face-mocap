...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        ...
        # 构建最后的分类层（这里默认为1000类别）
        self.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.last_channel, num_classes)    # num_classes
        )

        # # 后面再加三层全连接层，下采样到num_classes dim
    ...
    def forward(self, x):
        x = self.features(x)
        x = x.mean([2, 3])
        x = self.classifier(x)    # 这里到1000 dim
        # x = self.final_linear(x)    # 降维到 num_classees
        return x
...
if __name__ == '__main__':
    model = MobileNetV2(num_classes=52, output_stride=16)    # output_stride=8或16对加载预训练模型没影响
    # x = torch.randn([16, 3, 512, 512], dtype=torch.float32)
    # y = model(x)
    print(model)
    # print(y.shape)


    # 使用预训练模型（先改最后一层，再加载除最后一层的其他层）（也可以先完全加载，再修改最后一层）
    state_dict = torch.load("./checkpoint/mobilenet_v2-b0353104.pth")
    ...
    x = torch.rand([16, 3, 299, 299], dtype=torch.float32)  # 输入要求是299*299的
    y = model(x)
    print(y.shape)
